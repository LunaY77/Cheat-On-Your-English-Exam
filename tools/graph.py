...
def route_behavior(state: WritingState) -> Literal["write_introduction", "rewrite_content", "generate_content", "cite_content", "conclude_content", "full_content"]:
    ...
    if state.behavior == "topic":
        ...
        return "cite_content"
    elif state.behavior == "conclude":
        return "conclude_content"
    elif state.behavior == "full":
        return "full_content"
    else:
...
async def write_introduction(state: WritingState) -> dict[str, str]:
    ...
    messages = [
        {"role": "system",
         "content": """你是一个雅思写作 7 分水平的学生, 请保证你的词汇、语法、句型水平不要过于高级. 
         Now you are writing a academic essay on an IELTS test. 
         Write a concise and engaging introduction paragraph for the given topic. 
         Keep it under 100 words but above 60 words."""},
...
async def rewrite_content(state: WritingState) -> dict[str, str]:
    ...
    messages = [
        {"role": "system",
         "content": """你是一个雅思写作 7 分水平的学生, 请保证你的词汇、语法、句型水平不要过于高级. 
         Now you are writing a academic essay on an IELTS test. 
         Improve the writing quality of the given text without adding new content or changing the meaning. 
         Focus on grammar, word choice, and sentence structure."""},
...
async def generate_content(state: WritingState) -> dict[str, str]:
    ...
    messages = [
        {"role": "system",
         "content": """你是一个雅思写作 7 分水平的学生, 请保证你的词汇、语法、句型水平不要过于高级. 
         Now you are writing a academic essay on an IELTS test. 
         Add 1-2 lines of content that naturally follows the given text. 
...
async def cite_content(state: WritingState) -> dict[str, str]:
    ...
    messages = [
        {"role": "system",
         "content": """你是一个雅思写作 7 分水平的学生, 请保证你的词汇、语法、句型水平不要过于高级. 
            Now you are writing a academic essay on an IELTS test. Your task is to:
...
async def conclude_content(state: WritingState) -> dict[str, str]:
    ...
    messages = [
        {"role": "system",
         "content": """你是一个雅思写作 7 分水平的学生, 请保证你的词汇、语法、句型水平不要过于高级.
                    Now you are writing a academic essay on an IELTS test. 
                    Write a conclusion about the given article. 
                    Keep it under 100 words but above 50 words. 
                    Use phrase like 'In conclusion', 'To sum up', 'In summary' to start."""},
        {"role": "user", "content": f"Topic: {state.content}"}
    ]
    response = await model.ainvoke(messages)
    return {"response": response.content}

async def full_content(state: WritingState) -> dict[str, str]:
    """Generate a full essay based on the given topic."""
    model = load_chat_model()
    messages = [
        {"role": "system",
         "content": """你是一个雅思写作 7 分水平的学生, 请保证你的词汇、语法、句型水平不要过于高级. 
                    Now you are writing a academic essay on an IELTS test. 
                    Write a full essay based on the given topic. 
                    Include an introduction, body paragraphs, and a conclusion. 
                    Total words count should be between 500 and 600 words."""},
        {"role": "user", "content": f"Topic: {state.content}"}
    ]
    response = await model.ainvoke(messages)
    return {"response": response.content}
...
builder.add_node("full_content", full_content)
...
builder.add_edge("full_content", END)
